...
import { FileUp, FileText, Calendar as CalendarIcon, ChevronUp, ChevronDown, ChevronsUpDown } from "lucide-react";
...
type MonthlyData = { month: string; remPct: number; deepPct: number; total: number; awake: number; n: number; };
...
function SortableHeader({column, currentSort, direction, onSort, children, align = 'left'}:{
  ...
  return (
    <th 
      className={`px-3 py-2 cursor-pointer hover:bg-gray-100 select-none ${align === 'right' ? 'text-right' : 'text-left'}`}
      onClick={handleClick}
    >
...
function TimeSeriesChart({data, yKey, y2Key, meds}:{ data:any[]; yKey:string; y2Key?:string; meds?:MedEvent[]; }){
  return (<div className="h-72 w-full"><ResponsiveContainer width="100%" height="100%"><LineChart data={data} margin={{ top: 10, right: 20, left: 10, bottom: 0 }}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="dateStr" minTickGap={32} /><YAxis width={40} /><Tooltip formatter={(v:any)=> typeof v==="number"? v.toFixed(2): v} labelFormatter={(l:any)=>`Date: ${l}`} /><Legend />{meds?.map((m,i)=>(<ReferenceLine key={i} x={m.date.toISOString().slice(0,10)} stroke="#111827" strokeDasharray="4 4" label={{ position:'top', value:m.label, angle:-90, offset:10, fill:'#374151', fontSize:10 }} />))}<Line type="monotone" dataKey={yKey} name={yKey} stroke="#3b82f6" dot={false} strokeWidth={1.5} />{y2Key && <Line type="monotone" dataKey={y2Key} name={`${yKey} (rolling)`} stroke="#1f2937" dot={false} strokeWidth={2} />}<Brush dataKey="dateStr" height={18} travellerWidth={8} className="rounded" /></LineChart></ResponsiveContainer></div>);
}
function MedDeltaTable({meds, data}:{meds: MedEvent[]; data:SleepRow[]}){
  ...
  const rows = meds.map(m=>{
    ...
    const preWithPct = pre.map(r => ({ ...r, remPct: r.totalSleep ? (r.rem/r.totalSleep)*100 : 0, deepPct: r.totalSleep ? (r.deep/r.totalSleep)*100 : 0 }));
    const postWithPct = post.map(r => ({ ...r, remPct: r.totalSleep ? (r.rem/r.totalSleep)*100 : 0, deepPct: r.totalSleep ? (r.deep/r.totalSleep)*100 : 0 }));
    const before = { rem: avg(preWithPct,'remPct'), deep: avg(preWithPct,'deepPct'), total: avg(pre,'totalSleep'), awake: avg(pre,'awake') };
    const after  = { rem: avg(postWithPct,'remPct'), deep: avg(postWithPct,'deepPct'), total: avg(post,'totalSleep'), awake: avg(post,'awake') };
...
export default function App(){
  ...
  const enriched = useMemo(()=>{ const arr = filtered.map(r=>({ ...r, remPct: r.totalSleep? (r.rem/r.totalSleep)*100: 0, deepPct: r.totalSleep? (r.deep/r.totalSleep)*100: 0, corePct: r.totalSleep? (r.core/r.totalSleep)*100: 0 })); const remRoll = rolling(arr, rollK, x=>(x as any).remPct); const deepRoll = rolling(arr, rollK, x=>(x as any).deepPct); const totalRoll = rolling(arr, rollK, x=>(x as any).totalSleep); const awakeRoll = rolling(arr, rollK, x=>(x as any).awake); return arr.map((r,i)=> ({ ...r, dateStr: r.date.toISOString().slice(0,10), remRoll: remRoll[i], deepRoll: deepRoll[i], totalRoll: totalRoll[i], awakeRoll: awakeRoll[i] })); }, [filtered, rollK]);
  const chartData = useMemo(()=> (enriched as any).map((r:any)=>({ date: r.date, x: r.date.getTime(), dateStr: r.dateStr, remPct: r.remPct, remRoll: r.remRoll ?? null, deepPct: r.deepPct, deepRoll: r.deepRoll ?? null, awake: r.awake, awakeRoll: r.awakeRoll ?? null, total: r.totalSleep, totalRoll: r.totalRoll ?? null })), [enriched]);
  ...
  const monthlyData = useMemo(() => {
    ...
    const data: MonthlyData[] = groups.map(([ym, rows]) => {
      ...
      const remPct = validRows.length > 0 ? validRows.reduce((s, r) => s + (r.rem / r.totalSleep * 100), 0) / validRows.length : 0;
      const deepPct = validRows.length > 0 ? validRows.reduce((s, r) => s + (r.deep / r.totalSleep * 100), 0) / validRows.length : 0;
  ...
  return (<div className="min-h-screen bg-white text-gray-900 p-6 space-y-6"><header className="flex items-center justify-between"><h1 className="text-2xl font-semibold">Sleep Insights Explorer</h1><div className="flex items-center gap-3"><label className="inline-flex items-center gap-2 px-3 py-2 rounded-2xl bg-gray-100 hover:bg-gray-200 cursor-pointer"><FileUp className="w-4 h-4"/> Import Sleep (ZIP or CSV)<input type="file" accept=".zip,.csv" className="hidden" onChange={onUpload} /></label><label className="inline-flex items-center gap-2 px-3 py-2 rounded-2xl bg-gray-100 hover:bg-gray-200 cursor-pointer"><FileText className="w-4 h-4"/> Import meds (CSV or TXT)<input type="file" accept=".csv,.txt" className="hidden" onChange={onUploadMeds} /></label></div></header>{sleepRows.length===0 && (<div className="rounded-2xl border border-dashed p-8 text-center text-gray-600"><p className="text-lg">Default sleep data from 2024-2025 is loaded automatically. You can also upload your own Oura exports (ZIP of monthly CSVs or a single CSV) to override.</p><p className="mt-2 text-sm">Medications CSV is preloaded by default; you can also upload a meds CSV/TXT to override.</p><p className="mt-2 text-sm">We'll normalize the data, let you cap min/max sleep, pick date ranges, and overlay med changes.</p></div>)}{sleepRows.length>0 && (<><section className="sticky top-0 z-10 bg-white/95 backdrop-blur-sm border-b border-gray-200 py-4 -mx-6 px-6 mb-6"><section className="grid md:grid-cols-2 gap-4 items-end"><div className="grid grid-cols-2 sm:grid-cols-4 gap-3"><div className="p-3 rounded-2xl border"><label className="text-xs text-gray-500">Min Sleep (hrs)</label><input type="number" className="mt-1 w-full rounded-xl border px-3 py-2" value={minHours} min={0} max={24} step={0.25} onChange={e=>setMinHours(parseFloat(e.target.value))}/></div><div className="p-3 rounded-2xl border"><label className="text-xs text-gray-500">Max Sleep (hrs)</label><input type="number" className="mt-1 w-full rounded-xl border px-3 py-2" value={maxHours} min={0} max={24} step={0.25} onChange={e=>setMaxHours(parseFloat(e.target.value))}/></div><div className="p-3 rounded-2xl border"><label className="text-xs text-gray-500 flex items-center gap-2"><CalendarIcon className="w-4 h-4"/> From</label><input type="date" className="mt-1 w-full rounded-xl border px-3 py-2" value={dateFrom} onChange={e=>setDateFrom(e.target.value)}/></div><div className="p-3 rounded-2xl border"><label className="text-xs text-gray-500 flex items-center gap-2"><CalendarIcon className="w-4 h-4"/> To</label><input type="date" className="mt-1 w-full rounded-xl border px-3 py-2" value={dateTo} onChange={e=>setDateTo(e.target.value)}/></div></div><div className="grid grid-cols-2 gap-3"><div className="p-3 rounded-2xl border"><label className="text-xs text-gray-500">Rolling Avg (days)</label><input type="number" className="mt-1 w-full rounded-xl border px-3 py-2" value={rollK} min={1} max={30} step={1} onChange={e=>setRollK(parseInt(e.target.value||"1"))}/></div><div className="p-3 rounded-2xl border"><label className="text-xs text-gray-500">Nights Loaded</label><div className="text-xl font-semibold mt-1">{(filtered as any[]).length}</div></div></div></section></section><section className="grid gap-6"><ChartCard title="REM % Over Time" subtitle="Includes rolling average"><TimeSeriesChart data={chartData} yKey="remPct" y2Key="remRoll" meds={meds} /></ChartCard><ChartCard title="Deep % Over Time" subtitle="Includes rolling average"><TimeSeriesChart data={chartData} yKey="deepPct" y2Key="deepRoll" meds={meds} /></ChartCard><ChartCard title="Awake Hours Over Time" subtitle="Awake while in bed; includes rolling average"><TimeSeriesChart data={chartData} yKey="awake" y2Key="awakeRoll" meds={meds} /></ChartCard><ChartCard title="Total Sleep Duration" subtitle="Hover to see nightly totals; includes rolling average"><TimeSeriesChart data={chartData} yKey="total" y2Key="totalRoll" meds={meds} /></ChartCard></section><section className="space-y-3"><h2 className="text-lg font-semibold">Monthly Summary</h2><div className="overflow-auto rounded-2xl border"><table className="min-w-full text-sm"><thead className="bg-gray-50 text-gray-600"><tr><SortableHeader column="month" currentSort={sortColumn} direction={sortDirection} onSort={handleSort} align="left">Month</SortableHeader><SortableHeader column="remPct" currentSort={sortColumn} direction={sortDirection} onSort={handleSort} align="right">REM %</SortableHeader><SortableHeader column="deepPct" currentSort={sortColumn} direction={sortDirection} onSort={handleSort} align="right">Deep %</SortableHeader><SortableHeader column="total" currentSort={sortColumn} direction={sortDirection} onSort={handleSort} align="right">Total Sleep (hr)</SortableHeader><SortableHeader column="awake" currentSort={sortColumn} direction={sortDirection} onSort={handleSort} align="right">Awake (hr)</SortableHeader><SortableHeader column="n" currentSort={sortColumn} direction={sortDirection} onSort={handleSort} align="right">Nights tracked</SortableHeader></tr></thead><tbody>{monthlyData.map((data) => (<tr key={data.month} className="border-t"><td className="px-3 py-2">{data.month}</td><td className="px-3 py-2 text-right">{isFinite(data.remPct) ? data.remPct.toFixed(2) : '—'}</td><td className="px-3 py-2 text-right">{isFinite(data.deepPct) ? data.deepPct.toFixed(2) : '—'}</td><td className="px-3 py-2 text-right">{isFinite(data.total) ? data.total.toFixed(2) : '—'}</td><td className="px-3 py-2 text-right">{isFinite(data.awake) ? data.awake.toFixed(2) : '—'}</td><td className="px-3 py-2 text-right">{data.n}</td></tr>))}</tbody></table></div></section>{meds.length>0 && (<section className="space-y-2"><h2 className="text-lg font-semibold">Medication Change Deltas (30 days after − 30 days before)</h2><div className="text-sm text-gray-600">Hover vertical lines in the charts to see event labels.</div><MedDeltaTable meds={meds} data={sleepRows} /></section>)}</>)}<footer className="pt-8 text-xs text-gray-500">Built for exploring sleep-stage trends, vivid-dream spikes, and fatigue drivers. All data stays in your browser.</footer></div>);
}
